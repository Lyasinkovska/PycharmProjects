"""
The valid phone number program.

Make a program that checks if a string is in the right format for a phone number. The program should check that
the string contains only numerical characters and is only 10 characters long. Print a suitable message depending
on the outcome of the string evaluation.
"""

phone_number = input()
if phone_number.isnumeric():
	if len(phone_number) < 10:
		print("The number length is too short.")
	elif len(phone_number) > 10:
		print("The number length is too long.")
	else:
		print("The number is valid.")
else:
	print("The number contains not only numbers.")
